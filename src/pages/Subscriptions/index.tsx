...
import qs from 'qs'
import { flow, get, head, orderBy } from 'lodash/fp'
...
export default function Subscription(): JSX.Element {
  ...
  const [subSort, setSubSort] = useState<SubOrder>({ createdAt: SortDirection.Desc })
  ...
  const handleFilterChange = (params: GridFilterModel) => {
    setSubFilter(
      params.items.reduce((filter, { columnField, operatorValue, value }: GridFilterItem) => {
        ...
        if (columnField === 'paymentVersion' && value) {
          filter.paymentVersion = {
            [operatorValue as string]: +value,
          }
        }
  ...
  const subscriptions =
    subscriptionsData?.data.map((subscription) => {
      ...
      return {
        ...
        paymentStatus: latestPaymentEvent.status,
        paymentFailureMessage: latestPaymentEvent.omiseFailureMessage,
        paymentUpdatedDate: latestPaymentEvent.updatedAt,
      }
    }) || []
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      filterable: true,
      sortable: false,
      renderCell: ({ row }: GridCellParams) => row.paymentVersion ?? '-',
    },
    {
      field: 'createdAt',
    ...
    },
    {
      ...
      valueFormatter: (params: GridValueFormatterParams) =>
        columnFormatPaymentEventStatus(params.value as string, t),
      flex: 1,
      filterable: false,
